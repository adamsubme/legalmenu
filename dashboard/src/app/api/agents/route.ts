...
export async function POST(request: NextRequest) {
  try {
    ...
    run(
      `INSERT INTO agents (id, name, role, description, avatar_emoji, is_master, workspace_id, soul_md, user_md, agents_md, heartbeat_md, created_at, updated_at)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
      [
        id,
        body.name,
        ...
        body.user_md || null,
        body.agents_md || null,
        (body as { heartbeat_md?: string }).heartbeat_md || null,
        now,
        now,
      ]
    );
