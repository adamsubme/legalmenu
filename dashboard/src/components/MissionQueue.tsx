...
const COLUMNS: { id: TaskStatus; label: string; color: string }[] = [
  { id: 'not_started', label: 'NOT STARTED', color: 'border-t-mc-accent-pink' },
  { id: 'in_progress', label: 'IN PROGRESS', color: 'border-t-mc-accent' },
  { id: 'blocked', label: 'BLOCKED', color: 'border-t-mc-accent-red' },
  { id: 'awaiting_approval', label: 'AWAITING APPROVAL', color: 'border-t-mc-accent-yellow' },
              ...
              <div className="flex-1 overflow-y-auto p-2 space-y-2">
                {columnTasks.map((task) => (
                  <TaskCard
                    ...
                    onDispatch={(e) => { e.stopPropagation(); handleDispatch(task); }}
...
function TaskCard({ task, onDragStart, onClick, onDispatch, isDragging }: TaskCardProps) {
  ...
  const isPlanning = !!task.planning_session_key && !task.planning_complete;
  ...
  return (
    ...
    >
      ...
      <div className="p-4">
        ...
        {task.assigned_agent && (
          <div className="flex items-center justify-between gap-2 mb-3 py-1.5 px-2 bg-mc-bg-tertiary/50 rounded">
            ...
            {task.status === 'not_started' && onDispatch && (
              <button
                onClick={onDispatch}
                className="flex-shrink-0 p-1 rounded hover:bg-mc-accent/20 text-mc-accent"
                title="Wyślij do agenta"
              >
