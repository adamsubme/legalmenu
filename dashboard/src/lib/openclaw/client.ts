...
export class OpenClawClient extends EventEmitter {
  ...
  async getChatHistory(sessionKey: string, limit = 100): Promise<{ role: string; content: string }[]> {
    const result = await this.call<{ messages?: { role: string; content: string }[] }>('chat.history', {
      sessionKey,
      limit,
    });
...
// Singleton instance for server-side usage (created at runtime, not build)
